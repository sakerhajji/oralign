...
export const usersService = {
   ...
   * Upload user avatar.
   *
   * IMPORTANT: do NOT set the `Content-Type` header manually. The
   * multipart boundary is generated by the browser when axios serialises
   * the FormData body, and hardcoding `multipart/form-data` strips that
   * boundary — so the server receives a body it can't parse and
   * `@UploadedFile()` ends up `undefined`. Letting axios handle the
   * header preserves the boundary parameter and the upload works.
   *
   * This is the same fix Nest + Multer require everywhere FormData is
   * used; the bug surfaced first on the post-register onboarding photo
   * step but the previous header was wrong everywhere.
   */
