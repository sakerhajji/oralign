...
import { Check, Edit3, Eye, Heart, Loader2, Plus, RefreshCw, Trash2, X } from 'lucide-react';
...
type Queue = CommunitySubmissionStatus;
...
export function AdminCommunityContent() {
  ...
  const list = useCommunitySubmissionsAdmin({ status: queue, page: 1, limit: 50 }, isAdmin);
      ...
      <Card className="min-w-0 overflow-hidden">
        ...
        <CardContent className="p-3 sm:p-5 lg:p-6">
          ...
          <div className="grid gap-4">
            {items.map((item) => <SubmissionCard key={item.id} item={item} onPreview={() => setSelected(item)} onEdit={() => { setEditing(item); setEditorOpen(true); }} onApprove={() => approve.mutate(item.id)} onReject={() => { setSelected(item); setReviewNote(''); setRejectOpen(true); }} onDelete={() => remove.mutate(item.id)} busy={approve.isPending || reject.isPending || remove.isPending} />)}
          </div>
        </CardContent>
      </Card>
...
function SubmissionCard({ item, onPreview, onEdit, onApprove, onReject, onDelete, busy }: { item: CommunitySubmission; onPreview: () => void; onEdit: () => void; onApprove: () => void; onReject: () => void; onDelete: () => void; busy: boolean }) {
  const { t } = useT();
  ...
  return (
    <article className="grid min-w-0 gap-4 rounded-xl border bg-card p-3 shadow-sm transition-colors hover:border-primary/30 sm:grid-cols-[176px_minmax(0,1fr)_auto] sm:p-4">
      ...
      <div className="min-w-0 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">{item.city || '—'} · {t('communityAdmin.submittedOn', { date })}</p>
