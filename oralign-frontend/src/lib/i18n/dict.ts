...
export const dict = {
  ...
  accountBillingSettings: {
    ...
    beyondPackTitle: { en: 'Beyond-the-pack tariffs', fr: 'Tarifs au-delà du forfait' } as T,
    beyondPackHelp: {
      en: 'Grille 2026 extras shown to practitioners. New values apply to future cases only.',
      fr: 'Suppléments de la grille 2026 affichés aux praticiens. Les nouvelles valeurs ne concernent que les cas futurs.',
    } as T,
    refinementTwoLabel: { en: 'Extra refinement — two arches ({currency})', fr: 'Finition supplémentaire — deux arcades ({currency})' } as T,
    refinementOneLabel: { en: 'Extra refinement — one arch ({currency})', fr: 'Finition supplémentaire — une arcade ({currency})' } as T,
    replacementLabel: { en: 'Replacement aligner — one arch ({currency})', fr: 'Gouttière de remplacement — une arcade ({currency})' } as T,
    retainersLabel: { en: 'Retainers — 2 sets ({currency})', fr: 'Contention — lot de 2 jeux ({currency})' } as T,
    loyaltyEnabledLabel: { en: 'Loyalty program', fr: 'Programme de fidélité' } as T,
    loyaltyEnabledHelp: {
      en: 'Quarterly discounts for loyal practitioners. Turning this off stops new discounts; issued quotes keep theirs.',
      fr: 'Remises trimestrielles pour les praticiens fidèles. Désactiver arrête les nouvelles remises ; les devis émis conservent la leur.',
    } as T,
  ...
  packsAdmin: {
    ...
    tabPacks: { en: 'Packs', fr: 'Forfaits' } as T,
    tabLoyalty: { en: 'Loyalty', fr: 'Fidélité' } as T,
    beyondPackTitle: { en: 'Beyond the pack', fr: 'Au-delà du forfait' } as T,
    beyondPackIntro: {
      en: 'Tariffs applied outside the bundle. Edit them in Billing settings — new values apply to future cases only.',
      fr: 'Tarifs appliqués hors forfait. Modifiables dans les Réglages de facturation — les nouvelles valeurs ne concernent que les cas futurs.',
    } as T,
    beyondPackStudy: { en: 'Preliminary study + 3D treatment plan', fr: 'Étude et plan de traitement 3D' } as T,
    beyondPackStudyHint: {
      en: 'Fully deducted from the pack price once the case starts.',
      fr: 'Intégralement déduite du prix du forfait dès que le cas est lancé.',
    } as T,
    beyondPackDicom: { en: 'DICOM / cone-beam supplement', fr: 'Supplément fichiers DICOM — cone beam' } as T,
    beyondPackRefinementTwo: { en: 'Extra refinement — two arches', fr: 'Finition supplémentaire — deux arcades' } as T,
    beyondPackRefinementOne: { en: 'Extra refinement — one arch', fr: 'Finition supplémentaire — une arcade' } as T,
    beyondPackReplacement: { en: 'Replacement aligner — one arch', fr: 'Gouttière de remplacement — une arcade' } as T,
    beyondPackRetainers: { en: 'Retainers — 2 sets, two arches', fr: 'Gouttières de contention — lot de 2 jeux, deux arcades' } as T,
    beyondPackEdit: { en: 'Edit in billing settings', fr: 'Modifier dans les réglages' } as T,
    beyondPackNotSet: { en: 'Not set', fr: 'Non renseigné' } as T,
    beyondPackDicomOff: { en: 'Supplement disabled', fr: 'Supplément désactivé' } as T,
    beyondPackLoadError: { en: 'Tariffs could not be loaded.', fr: 'Impossible de charger les tarifs.' } as T,
    beyondPackRetry: { en: 'Retry', fr: 'Réessayer' } as T,
  ...
  // ─── Loyalty program (admin tab on /dashboard/packs) ─────────────
  loyaltyAdmin: {
    title: { en: 'Loyalty program', fr: 'Programme de fidélité' } as T,
    intro: {
      en: 'Treatments approved during a quarter earn a discount on every pack quote of the following quarter. Recomputed at each quarter end.',
      fr: 'Les traitements validés pendant un trimestre ouvrent une remise sur tous les devis forfait du trimestre suivant. Recalcul à chaque fin de trimestre.',
    } as T,
    disabledBanner: {
      en: 'The loyalty program is currently disabled in Billing settings — no discount is applied to new quotes.',
      fr: 'Le programme de fidélité est désactivé dans les Réglages de facturation — aucune remise n’est appliquée aux nouveaux devis.',
    } as T,
    currentQuarter: { en: 'Current quarter', fr: 'Trimestre en cours' } as T,
    kpiTreatments: { en: 'Treatments this quarter', fr: 'Traitements ce trimestre' } as T,
    kpiEligible: { en: 'Practitioners at {percent}%', fr: 'Praticiens à {percent}%' } as T,
    kpiDiscounts: { en: 'Loyalty discounts granted', fr: 'Remises fidélité accordées' } as T,
    kpiDiscountsHint: { en: 'On quotes created this quarter', fr: 'Sur les devis créés ce trimestre' } as T,
    rulesTitle: { en: 'Tiers', fr: 'Paliers' } as T,
    rulesHint: {
      en: 'Editing tiers only affects future quarters — closed quarters keep the percentages they were awarded.',
      fr: 'Modifier les paliers ne touche que les trimestres futurs — les trimestres clôturés conservent leurs pourcentages.',
    } as T,
    ruleLine: {
      en: '{count}+ treatments / quarter → {percent}% next quarter',
      fr: '{count} traitements et plus / trimestre → {percent}% le trimestre suivant',
    } as T,
    editRules: { en: 'Edit tiers', fr: 'Modifier les paliers' } as T,
    editRulesTitle: { en: 'Loyalty tiers', fr: 'Paliers de fidélité' } as T,
    editRulesDesc: {
      en: 'Thresholds must increase and higher tiers cannot grant smaller discounts. Closed quarters are never rewritten.',
      fr: 'Les seuils doivent croître et un palier supérieur ne peut pas accorder une remise plus faible. Les trimestres clôturés ne sont jamais réécrits.',
    } as T,
    tierMinLabel: { en: 'Treatments / quarter', fr: 'Traitements / trimestre' } as T,
    tierPercentLabel: { en: 'Discount %', fr: 'Remise %' } as T,
    addTier: { en: 'Add a tier', fr: 'Ajouter un palier' } as T,
    removeTier: { en: 'Remove', fr: 'Retirer' } as T,
    saveRules: { en: 'Save tiers', fr: 'Enregistrer les paliers' } as T,
    recompute: { en: 'Recompute previous quarter', fr: 'Recalculer le trimestre précédent' } as T,
    tableTitle: { en: 'Practitioner progress', fr: 'Progression des praticiens' } as T,
    colPractitioner: { en: 'Practitioner', fr: 'Praticien' } as T,
    colProgress: { en: 'This quarter', fr: 'Ce trimestre' } as T,
    colCurrentReward: { en: 'Active discount', fr: 'Remise en cours' } as T,
    colNextQuarter: { en: 'Next quarter', fr: 'Trimestre suivant' } as T,
    colPrevious: { en: 'Previous quarter', fr: 'Trimestre précédent' } as T,
    progressCount: { en: '{count} / {target} treatments', fr: '{count} / {target} traitements' } as T,
    progressTopTier: { en: '{count} treatments — top tier reached', fr: '{count} traitements — palier maximal atteint' } as T,
    progressNoTiers: { en: '{count} treatments — no tiers configured', fr: '{count} traitements — aucun palier configuré' } as T,
    remainingHint: {
      en: '{remaining} more to unlock {percent}%',
      fr: 'Encore {remaining} pour débloquer {percent}%',
    } as T,
    activeDiscountHint: { en: 'Earned in {quarter}', fr: 'Acquise en {quarter}' } as T,
    securedNext: { en: '{percent}% secured', fr: '{percent}% acquis' } as T,
    noReward: { en: 'No discount', fr: 'Aucune remise' } as T,
    previousCell: { en: '{count} treatments → {percent}%', fr: '{count} traitements → {percent}%' } as T,
    emptyTitle: { en: 'No loyalty activity yet', fr: 'Aucune activité de fidélité pour le moment' } as T,
    emptyBody: {
      en: 'Practitioners appear here as soon as a pack quote is approved this quarter or a previous quarter was closed.',
      fr: 'Les praticiens apparaissent ici dès qu’un devis forfait est validé ce trimestre ou qu’un trimestre précédent a été clôturé.',
    } as T,
    loadError: { en: 'Could not load the loyalty overview.', fr: 'Impossible de charger l’aperçu fidélité.' } as T,
    retry: { en: 'Retry', fr: 'Réessayer' } as T,
    treatmentDefinition: {
      en: 'A treatment = a pack quote the practitioner approved during the quarter.',
      fr: 'Un traitement = un devis forfait validé par le praticien pendant le trimestre.',
    } as T,
  },
  ...
  toasts: {
    ...
    loyalty: {
      tiersUpdated: { en: 'Loyalty tiers updated.', fr: 'Paliers de fidélité mis à jour.' } as T,
      recomputed: { en: 'Quarter {quarter} recomputed.', fr: 'Trimestre {quarter} recalculé.' } as T,
    },
    patients: {
