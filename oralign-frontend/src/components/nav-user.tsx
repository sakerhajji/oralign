        ...
        <DropdownMenu onOpenChange={onOpenChange}>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              ...
              // icon rail): the footer slot is 40px wide (56px rail − the
              // footer's p-2). The old forced 48px tile OVERFLOWED that
              // slot (auto margins resolve to 0 on an over-wide child), so
              // the avatar sat visibly off the nav-icon axis. A 40px
              // (`!h-10 !w-10`) tile lines up dead on the same axis as
              // every rail icon.
